...
mix.sass(
    ...
    .js(
        ...
        `public/${demo}/plugins/global/plugins.bundle.js`,
    );

// Build extended plugin styles
mix.sass(
    `resources/assets/${demo}/sass/plugins.scss`,
    `public/${demo}/plugins/global/plugins-custom.bundle.css`,
);

// Build Metronic css/js
mix.sass(
    `resources/assets/${demo}/sass/style.scss`,
    ...
    .js(
        [
            `resources/assets/${demo}/js/scripts.js`,
            "resources/assets/extended/button-ajax.js",
        ],
        `public/${demo}/js/scripts.bundle.js`,
    );
...
if (args.indexOf("dark_mode") !== -1) {
    mix.sass(
        ...
        `public/${demo}/plugins/global/plugins.dark.bundle.css`,
    );
    mix.sass(
        `resources/assets/${demo}/sass/plugins.dark.scss`,
        `public/${demo}/plugins/global/plugins-custom.dark.bundle.css`,
    );
    mix.sass(
        `resources/assets/${demo}/sass/style.dark.scss`,
        `public/${demo}/css/style.dark.bundle.css`,
        { sassOptions: { includePaths: ["node_modules"] } },
    );
}
...
(glob.sync("resources/assets/extended/js/**/*.js") || []).forEach((file) => {
    var output = `public/${demo}/${file.replace(
...
(glob.sync(`resources/assets/${demo}/js/custom/**/*.js`) || []).forEach(
    (file) => {
        var output = `public/${demo}/${file.replace(
...
// Metronic theme
(glob.sync(`resources/assets/${demo}/sass/themes/**/!(_)*.scss`) || []).forEach(
    (file) => {
        file = file.replace(/[\\\/]+/g, "/");
        mix.sass(
            file,
            file
                .replace(`resources/assets/${demo}/sass`, `public/${demo}/css`)
                .replace(/\.scss$/, ".css"),
        );
    },
);
...
(
    glob.sync(`resources/assets/${demo}/plugins/**/*.+(woff|woff2|eot|ttf)`) ||
    []
).forEach((file) => {
    var folder = file.match(/resources\/metronic\/plugins\/(.*?)\//)[1];
    mix.copy(
        file,
        `public/${demo}/plugins/global/fonts/${folder}/${path.basename(file)}`,
    );
});
(
    glob.sync(
...
(glob.sync(`resources/assets/core/plugins/custom/**/*.js.json`) || []).forEach(
    (file) => {
        ...
        mix.scripts(
            filePaths,
            `public/${demo}/plugins/custom/${fileName}/${fileName}.bundle.js`,
        );
    },
);
