...
<div class="card {{ $class }}">
    ...
    <div class="card-header rotate cursor-pointer border-0" role="button" data-bs-toggle="collapse"
        ...
        <div class="card-title m-0">
            <h3 class="fw-bolder m-0">{{ __('Layanan Details') }}</h3>
        </div>
        <!--end::Card title-->
    </div>
    <!--begin::Card header-->
    ...
    <div id="kt_layanan_layanan_create" class="show collapse">
        ...
        <form id="kt_layanan_layanan_create_form" class="form" method="POST"
            action="{{ old('id', route('layanan.update', $layanan->id) ?? route('layanan.store')) }}"
                    ...
                    <div class="col-lg-4 fv-row">
                        <select name="type" aria-label="{{ __('Pilih Jenis Layanan') }}" data-control="select2"
                            ...
                            <option value="KENDARAAN"
                                {{ 'KENDARAAN' == old('type', $layanan->type ?? '') ? 'selected' : '' }}>
                                {{ __('Kendaraan') }}
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="col-lg-4 fv-row">
                        <select name="location" aria-label="{{ __('Pilih Lokasi') }}" data-control="select2"
                            ...
                            <option value="GANESHA"
                                {{ 'GANESHA' == old('location', $layanan->location ?? '') ? 'selected' : '' }}>
                                {{ __('GANESHA') }}
                            </option>
                            <option value="SARAGA"
                                {{ 'SARAGA' == old('location', $layanan->location ?? '') ? 'selected' : '' }}>
                                {{ __('SARAGA') }}
                            </option>
                            <option value="JATINANGOR"
                                {{ 'JATINANGOR' == old('location', $layanan->location ?? '') ? 'selected' : '' }}>
                                {{ __('JATINANGOR') }}</option>
                            <option value="CIREBON"
                                {{ 'CIREBON' == old('location', $layanan->location ?? '') ? 'selected' : '' }}>
                                {{ __('CIREBON') }}</option>
                        </select>
                    </div>
                    <!--end::Col-->
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 fv-row">
                                <input type="text" id="price" name="price"
                                    ...
                                    placeholder="Harga" value="{{ old('price', $layanan->price ?? '') }}" />
                            </div>
                            <!--end::Col-->
                            ...
                            <div class="col-lg-5 fv-row">
                                <select name="price_for" aria-label="{{ __('Pilih Harga Per') }}"
                                    data-control="select2" data-placeholder="{{ __('Pilih Harga Per...') }}"
                                    class="form-select form-select-solid form-select-lg fw-bold">
                                    <option value="JAM"
                                        {{ 'JAM' == old('price_for', $layanan->price_for ?? '') ? 'selected' : '' }}>
                                        {{ __('Jam') }}</option>
                                    <option value="HARI"
